...
public class DList {
  ...
  public String chipToString() {
    ...
    DListNode current = head.next;
    while (current != head) {
      result = result + "(" + ((Chip)current.item).x + ", " + ((Chip)current.item).y + ")" + "  ";
      current = current.next;
    }
    return result + "]";
  }

  public String addmoveToString() {
    String result = "[  ";
    DListNode current = head.next;
    while (current != head) {
      result = result + "(" + ((Move)current.item).x1 + ", " + ((Move)current.item).y1 + ")" + "  ";
      current = current.next;
    }
    return result + "]";
  }

  public String stepmoveToString() {
    String result = "[  ";
    DListNode current = head.next;
    while (current != head) {
      result = result + "(" + ((Move)current.item).x2 + ", " + ((Move)current.item).y2 + ")" + "  to " + 
      "(" + ((Move)current.item).x1 + ", " + ((Move)current.item).y1 + ")" + " || ";
      current = current.next;
    }
    return result + "]";
  }
